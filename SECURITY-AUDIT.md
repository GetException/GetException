...
Для удаления проектов `getexception_worker` может вызвать только `purge_deleted_project_batch()`: функция без аргументов выбирает проект с истёкшим семидневным сроком восстановления, удаляет данные порциями и сохраняет факт очистки в audit log. Прямое удаление `project` этой роли не разрешено. Функция имеет фиксированный `search_path`, включая `pg_temp` последним, и недоступна `PUBLIC`, ingest и web. Прикладные операции изменения, удаления и восстановления требуют Owner, CSRF-проверку и свежий TOTP step-up; удаление дополнительно сверяет текущий slug на сервере.
...
## Удостоверения GitLab для MR (2026-09-17)

Постоянные upload tokens остаются только в доверенном CI. Для MR добавляется отдельный метод `Authorization: GitLab <ID token>`: локальная проверка RS256 через jose по операторскому JWKS, без internet-egress, discovery из запроса или доступа к произвольным URL. Интеграция выключена по умолчанию. Проверяются issuer, единственный audience, срок до часа, project ID/path источника и выполнения, SHA/config ref; неизвестные/просроченные ключи и неподдерживаемые claims дают отказ без fallback.

Все операции с картами ограничены release и отдельным путём job, а регистрация release — разрешённым окружением. Production требует явно разрешённый protected ref и не-MR pipeline. Worker сверяет путь даже с Debug ID; совпадение ID в другом пути не заменяет существующие карты. Доверие к issuer не означает доверия к содержимому произвольного MR: автор может прислать неверную карту своей сборки или похитить её краткоживущее удостоверение. Он не должен получать постоянные credentials или право чтения других данных. Размеры, quotas, изолированный parser и закрытый storage остаются обязательными.

Открытые карты не сохраняются в GitLab artifacts/cache при непроверенных ACL. Существующие архивы от прежнего pipeline не удаляются изменением кода. Перед фактическим включением нужны установка operator policy, опубликованный CLI, проверка настоящего GitLab job и результата symbolication; локальные тесты не заменяют эту приёмку. Полный порядок и ограничения: [GitLab CI](docs/gitlab-ci.md).
...
Дополнительные источники разрешены только для MR preview. Подписанные source и execution projects проверяются отдельно: job может исполняться в разрешённом источнике либо основном репозитории. Null CI config принимается только парой в fork MR при явном подписанном execution project основного репозитория; остальные противоречия отвергаются. GitLab подпись не доказывает target MR или review: Owner доверяет сборкам источника целиком. Администраторы этого GitLab, авторы CI кода разрешённого форка и runner остаются границей доверия; временный токен не делает вредоносную сборку безопасной.

`ci context` v2 явно отличает policy skip от ошибок подписи/сервера. Неизвестный источник получает только skip, без контекста, регистрации release или upload. Выключенный preview у доверенного источника сохраняет регистрацию staging, но запрещает все map operations. Политика перечитывается на каждом запросе, включая finalize/status; операция, начавшаяся до отзыва, может завершиться. Существующие карты не удаляются. Постоянные Bearer tokens запрещены у GitLab-bound проектов, чтобы нельзя было обойти режим production-only. Ни источник из списка, ни skip не дают чтения карт/ошибок/настроек.

Account обязан подтвердить совместимость с v2 до применения: явный skip строит без карт, ошибка не маскируется как skip; публичная сборка, CI artifacts/cache проверяются на отсутствие карт в обоих режимах. Локальные подписи/БД/browser tests не заменяют приёмку реального fork pipeline после выпуска.

## Ошибки мониторинга и доступность deploy (2026-09-18)
