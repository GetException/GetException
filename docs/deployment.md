...
2. Push в `stable` запускает `Prepare release`. Workflow повышает patch-версию обоих SDK через Yarn, выполняет полный `yarn checks`, проверяет состав изменений и секреты, затем отправляет commit версий с `[skip ci]` и `Release-Source`. В него входят только manifests двух SDK и изменения lockfile, созданные Yarn.
3. `Prepare release` запускает `Release` на точном SHA этого commit. Повторные проверки выполняются уже на окончательном commit, на который будут ссылаться npm provenance, образы и установщик. Проверка remote `stable` блокирует публикацию и deploy устаревшего commit.
4. После проверок параллельно публикуются npm-пакеты `@getexception/browser` и `@getexception/react` с provenance и собираются пять Linux amd64 образов: web, ingest, worker, mail и migrate. Образы публикуются в GHCR с SHA, SBOM и provenance. Критические уязвимости блокируют продолжение. Только шаг npm-публикации получает `NPM_TOKEN`.
5. CI скачивает опубликованные npm tarballs, сверяет их с архивами из этого checkout и собирает отдельные browser/React fixtures. В GitHub Releases появляется prerelease `deploy-<полный SHA>` с установщиком, bundle, checksums и attestations.
6. Общий тест скачивает опубликованный установщик и запускает временную Docker-установку с опубликованными SDK. Он проверяет setup с TOTP, приём ошибок, UI, обновление, откат, восстановление backup и сохранение данных. Только после успеха prerelease становится обычным release.
7. Если `DEPLOY_ENABLED=true`, CI автоматически подключается к существующему серверу по SSH и обновляет его до того же SHA. Перед миграцией создаётся backup; после запуска проверяются HTTPS и обработка события worker. Owner, пароль, MFA и данные сохраняются. Дополнительного ручного запуска для каждого push не требуется.

## Повторный запуск выпуска

При сбое повторите упавшие jobs через GitHub Actions. Можно также вручную запустить `Prepare release` на актуальной `stable`: если там уже подготовленный commit версий, он будет использован повторно без нового повышения версии. `Release` принимает только актуальный подготовленный commit; необязательный `release_sha` дополнительно фиксирует ожидаемый полный SHA.

Если за время проверки появился новый обычный commit в `stable`, старый выпуск останавливается; новый push готовит следующую patch-версию. Существующие npm-версии и образы того же SHA сверяются и переиспользуются, артефакты не перезаписываются. Уже опубликованный пакет нельзя отменить при последующем сбое тестов, но сервер до успешного завершения цепочки не обновляется.
...
- Создайте environment `release`, разрешив deployment только из ветки `stable` (Selected branches and tags). Для автоматической доставки не добавляйте ручной approval. Добавьте `NPM_TOKEN` с правом публикации двух SDK в repository secrets либо secrets этого environment. Workflow передаёт токен только шагу публикации. Разрешите GitHub Actions публиковать в npm от имени владельца scope `@getexception`.
- Для подготовки выпуска добавьте публичную часть отдельного SSH-ключа в **Settings → Deploy keys** с флагом **Allow write access**, а приватную часть целиком — в repository secret `DEPLOY_KEY`. Workflow использует этот ключ только для подготовки и push release commit. Для защищённой `stable` её правила должны разрешать такой commit; право записи ключа само по себе не обходит ruleset. `RELEASE_TOKEN` не нужен: встроенный `GITHUB_TOKEN` с `Actions: write` используется только для вызова `Release`. Обычные изменения проходят PR/review.
- Создайте environment `production` с разрешением только для ветки `stable`, без дополнительного ручного approval при включённой автоматической доставке. Параллельные выпуски сериализованы через workflow concurrency, на сервере действует дополнительный lock.
...
Перед SSH job скачивает bundle и публичные доказательства подписи, проверяет checksum, репозиторий, workflow и SHA. На сервер передаются только attestation и trusted root в `runtime/deployments/<SHA>/`; установленный controller повторяет проверку перед обновлением. GitHub- и npm-токены на сервер не передаются. SSH использует строгую проверку host key. CI не выполняет `ssh-keyscan` с автоматическим доверием и не создаёт новый Owner на сервере.
...
Один deploy-пользователь должен иметь доступ к Docker и запись в каталог установки. Например, оператор заранее создаёт `/opt/getexception` с владельцем этим пользователем и правами `0700`. Последующие команды выполняются от того же пользователя. Для первой установки используйте публичные доказательства подписи по инструкции ниже либо настройте `gh auth login` с минимальным доступом для онлайн-проверки attestations. SMTP можно подключить позже; для внешней доставки нужен существующий почтовый сервис с TLS или STARTTLS. Тестовая почта в production не запускается.
...
Запускайте bootstrap только после успешной проверки checksum и подписи. Оба параметра offline-проверки передаются вместе; controller проверяет архив с теми же ограничениями identity. Токены GitHub на сервер не передаются. Доступ к интернету для скачивания артефактов, образов и HTTPS-сертификатов всё ещё нужен. При автоматическом обновлении свежие доказательства передаёт CI. При ручном обновлении передайте их самостоятельно и укажите те же параметры команде `getexception update` либо настройте онлайн-проверку через `gh auth login`.
...
CI выполняет эту команду с дополнительными `--attestation-bundle` и `--trusted-root`, указывающими на переданные публичные доказательства. При ручном обновлении без GitHub-токена также укажите эти два параметра. Она отправляет одно техническое событие в probe-проект и ждёт его обработки worker. Если проверка не проходит, обновление считается неудачным и выполняется совместимый откат. Первую настройку probe-проекта делаем после создания Owner, перед включением `DEPLOY_ENABLED`.
