...
При включённых картах попытаться получить `ready` всех batches в пределах общего бюджета времени. Только тогда сообщать об успешной доставке карт. Ошибка или timeout upload/ready не препятствуют deploy подготовленного JS. Во **всех** режимах проверить отсутствие `.map`, `.map.gz`, `.map.br`, inline source maps, credentials и приватных каталогов в публичной сборке. Попытаться зарегистрировать release с `deployment` из ответа `ci context` после успешного deploy только если сервер вернул этот контекст и остался сетевой бюджет. Ошибка регистрации не меняет статус deploy. Отзыв доступа после initialize прекращает разрешённые операции с картами, но не обычный deploy; не выдавать такой upload за ready.
...
- Управляющий код задаёт жёсткий timeout дочернему CLI: context/register до 10 секунд; суммарный бюджет **сетевых** вызовов GetException до 60 секунд на job (время обычной сборки в него не входит). Каждый upload ограничен оставшимся бюджетом, batches делят один бюджет. По исчерпании новых запросов нет; дочерние процессы завершаются без утечки credentials. Локальный prepare также имеет конечный timeout. Эти ограничения действуют независимо от внутренних retry CLI.
