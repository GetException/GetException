...
Push в `stable` запускает проверки и подготовку версии. Release workflow публикует готовые образы и SDK, затем проверяет опубликованный установщик. Автоматическое подключение к серверу включается отдельно после первой установки.
...
Нужны Node **24.4.0**, Corepack, Yarn **4.17.1** и установленный оператором Docker с Compose v2. Используется `node_modules`, PnP запрещён.
